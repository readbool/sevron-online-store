...
final class CheckoutController extends AbstractBaseController
{
    ...
    public function __invoke(Request $request): JsonResponse
    {
        ...
        if ($order->getStatus() === OrderStatusEnum::CANCELLED->value ||
            ...
            $order->getStatus() === OrderStatusEnum::PAID->value
        ) {
        ...
        $updatedOrder = $this->orderStatusUpdater->updateStatus($order, OrderStatusEnum::PAID);

        return $this->sendResponse($updatedOrder->load('products'), 'Order paid.');
    }
}
